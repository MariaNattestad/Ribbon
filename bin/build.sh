...
# Note that we use `awk 1` instead of `cat` so we append an extra line after each file. Otherwise,
# this could break if one of the modules doesn't have a break line at the end of the min.js files.
